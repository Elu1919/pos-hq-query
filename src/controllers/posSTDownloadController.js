...
const posDataController = {
  ...
  downloadPerformanceDataOK: async (req, res) => {
    const CONFIG = {
      DEFAULT_FONT: '微軟正黑體',
      ROW_HEIGHT_DEF: 22,
      UNIT_CONVERSION: 5.5,
    ...
    const LINE_BREAK_CONFIG = {
      ...
      // 字元權重設定 (數值越大越容易觸發換行)
      CHAR_WEIGHT_FULL: 0.4,  // 全形字元 (中文、全形標點)
      CHAR_WEIGHT_HALF: 0.2,  // 半形字元 (英數、半形標點)

      // 寬度利用率縮放因子 (0.95 代表達到欄位寬度的 95% 就換行，留一點緩衝避免文字貼邊)
      SENSITIVITY: 0.98
    }
    ...
    const calculateLines = (text, targetCm) => {
      ...
      lines.forEach(line => {
        let vLen = 0
        for (let i = 0; i < line.length; i++) {
          // 判斷 Unicode 編碼是否大於 255
          vLen += line.charCodeAt(i) > 255
            ? LINE_BREAK_CONFIG.CHAR_WEIGHT_FULL
            : LINE_BREAK_CONFIG.CHAR_WEIGHT_HALF
        }
        totalLines += Math.ceil(vLen / (targetCm * LINE_BREAK_CONFIG.SENSITIVITY)) || 1
      })
      return totalLines
    ...
    try {
      ...
      ws.pageSetup.margins = {
        top: CONFIG.MARGIN_TOP / CONFIG.INCH_TO_CM,
        bottom: CONFIG.MARGIN_BOTTOM / CONFIG.INCH_TO_CM,
        left: CONFIG.MARGIN_LEFT / CONFIG.INCH_TO_CM,
        right: CONFIG.MARGIN_RIGHT / CONFIG.INCH_TO_CM,
        header: CONFIG.MARGIN_HEADER / CONFIG.INCH_TO_CM,
        footer: CONFIG.MARGIN_FOOTER / CONFIG.INCH_TO_CM
      }
      ...
      ws.properties.defaultRowHeight = CONFIG.ROW_HEIGHT_DEF
      COL_WIDTHS.forEach(item => {
        const col = ws.getColumn(item.col)
        col.width = item.cm * CONFIG.UNIT_CONVERSION
        col.font = STYLES.FONT_NORMAL
      })
      ...
      for (let i = 0; i < shops.length; i++) {
        ...
        for (let r = 0; r < CONFIG.DETAIL_START_OFFSET; r++) {
          ws.getRow(pageStart + r).height = CONFIG.ROW_HEIGHT_DEF
        }

        const row1 = ws.getRow(pageStart)
        row1.getCell(1).value = '銷貨門市:'; row1.getCell(1).alignment = { horizontal: 'left', vertical: 'middle' }
        row1.getCell(2).value = shopName; row1.getCell(2).alignment = { horizontal: 'left', vertical: 'middle' }
        row1.getCell(7).value = '業績日期:'; row1.getCell(7).alignment = { horizontal: 'right', vertical: 'middle' }
        row1.getCell(8).value = `${filterIn.SALE_DATE_S} ~ ${filterIn.SALE_DATE_E}`; row1.getCell(8).alignment = { horizontal: 'left', vertical: 'middle' }

        const statHIdx = pageStart + 1
        const statHeaders = [
          { c: 1, v: '業績統計' }, { c: 2, v: '門市客' }, { c: 4, v: '保養廠' },
          { c: 6, v: '機車行' }, { c: 8, v: '業務' }, { c: 10, v: '盤商' }, { c: 12, v: '合計' }
        ]
        statHeaders.forEach(h => {
          const cell = ws.getCell(statHIdx, h.c)
          cell.value = h.v; if (h.c >= 2) ws.mergeCells(statHIdx, h.c, statHIdx, h.c + 1)
          cell.fill = STYLES.FILL_BLACK; cell.font = STYLES.FONT_WHITE_BOLD; cell.border = STYLES.BORDER; cell.alignment = { horizontal: 'center', vertical: 'middle' }
        })

        let groupTotals = {}
        PAY_MAPPING.forEach(p => {
          const rIdx = pageStart + p.rowOffset; const row = ws.getRow(rIdx)
          const nCell = row.getCell(1)
          nCell.value = p.name; nCell.border = STYLES.BORDER; nCell.alignment = { horizontal: 'left', vertical: 'middle', indent: CONFIG.INDENT_LEFT }
          nCell.font = p.name === 'iPASS MONEY' ? STYLES.FONT_NORMAL_8 : STYLES.FONT_NORMAL
          let rSum = 0
          GRP_MAPPING.forEach(g => {
            const val = shopData.filter(d => p.payIds.includes(d.PAY_ID) && d.VIPGRP_ID === g.id && d.PAY_ID !== '6').reduce((acc, cur) => acc + (Number(cur.TOTAL) || 0), 0)
            ws.mergeCells(rIdx, g.col, rIdx, g.col + 1); const cell = row.getCell(g.col)
            cell.value = val; cell.border = STYLES.BORDER; cell.alignment = { horizontal: 'right', vertical: 'middle', indent: CONFIG.INDENT_RIGHT }
            rSum += val; groupTotals[g.id] = (groupTotals[g.id] || 0) + val
          })
          ws.mergeCells(rIdx, 12, rIdx, 13); row.getCell(12).value = rSum; row.getCell(12).border = STYLES.BORDER; row.getCell(12).alignment = { horizontal: 'right', vertical: 'middle', indent: CONFIG.INDENT_RIGHT }
        })

        const totalIdx = pageStart + 7; const tRow = ws.getRow(totalIdx)
        tRow.getCell(1).value = '總計'; tRow.getCell(1).font = STYLES.FONT_RED_BOLD; tRow.getCell(1).border = STYLES.BORDER; tRow.getCell(1).alignment = { horizontal: 'left', vertical: 'middle', indent: CONFIG.INDENT_LEFT }
        let gTotal = 0
        GRP_MAPPING.forEach(g => {
          ws.mergeCells(totalIdx, g.col, totalIdx, g.col + 1); const cell = tRow.getCell(g.col)
          cell.value = groupTotals[g.id] || 0; cell.font = STYLES.FONT_RED_BOLD; cell.border = STYLES.BORDER; cell.alignment = { horizontal: 'right', vertical: 'middle', indent: CONFIG.INDENT_RIGHT }
          gTotal += (groupTotals[g.id] || 0)
        })
        ws.mergeCells(totalIdx, 12, totalIdx, 13); tRow.getCell(12).value = gTotal; tRow.getCell(12).font = STYLES.FONT_RED_BOLD; tRow.getCell(12).border = STYLES.BORDER; tRow.getCell(12).alignment = { horizontal: 'right', vertical: 'middle', indent: CONFIG.INDENT_RIGHT }

        const headIdx = pageStart + 9
        const labels = { 1: '日期', 15: '數量', 16: '單位', 17: '加值', 18: '單價', 19: '總折讓', 20: '小計', 21: '招待備註', 22: '結帳方式', 23: '發票' }
        for (let c = 1; c <= 23; c++) {
          const cell = ws.getCell(headIdx, c)
          cell.fill = STYLES.FILL_BLACK; cell.font = STYLES.FONT_WHITE_BOLD; cell.border = STYLES.BORDER; cell.alignment = { horizontal: 'center', vertical: 'middle' }
          if (labels[c]) cell.value = labels[c]
        }
        DETAIL_MERGE_CONFIGS.forEach(m => { ws.mergeCells(headIdx, m.start, headIdx, m.end); ws.getCell(headIdx, m.start).value = m.label })

        let detIdx = pageStart + CONFIG.DETAIL_START_OFFSET
        GROUP_CONFIGS.forEach(grp => {
          const items = shopData.filter(d => d.VIPGRP_ID === grp.id && d.PAY_ID !== '6')
          if (items.length > 0) {
            ws.mergeCells(detIdx, 1, detIdx, 23); const tCell = ws.getCell(detIdx, 1)
            tCell.value = grp.label; tCell.font = STYLES.FONT_BLACK_BOLD; tCell.fill = STYLES.FILL_GRAY; tCell.border = STYLES.BORDER; tCell.alignment = { horizontal: 'left', vertical: 'middle', indent: CONFIG.INDENT_LEFT }
            ws.getRow(detIdx).height = CONFIG.ROW_HEIGHT_DEF
            detIdx++
            ...
            items.forEach(item => {
              ...
              const maxL = Math.max(vL, pL, mL)
              row.height = maxL > 1 ? maxL * CONFIG.ROW_HEIGHT_DEF : CONFIG.ROW_HEIGHT_DEF
              ...
              DETAIL_MERGE_CONFIGS.forEach(m => ws.mergeCells(detIdx, m.start, detIdx, m.end))
              for (let c = 1; c <= 23; c++) {
                const cell = row.getCell(c); cell.border = STYLES.BORDER; cell.font = STYLES.FONT_NORMAL
                if (c === 22 && ['iPASS MONEY', '賒帳(不開發票)', '信用卡(已付款)'].includes(item.PAY_NAEM)) cell.font = STYLES.FONT_NORMAL_10
                if (DETAIL_MERGE_CONFIGS.some(m => c > m.start && c <= m.end)) continue
                let alH = 'center'; if ([2, 8, 21, 22].includes(c)) alH = 'left'; if ([15, 18, 19, 20].includes(c)) alH = 'right'
                cell.alignment = { vertical: 'middle', horizontal: alH, wrapText: true, indent: alH === 'left' ? CONFIG.INDENT_LEFT : (alH === 'right' ? CONFIG.INDENT_RIGHT : 0) }
              }
              detIdx++
            })
        ...
        currentRow = detIdx; if (i < shops.length - 1) ws.getRow(currentRow - 1).addPageBreak()
      }
      ws.pageSetup.printArea = `A1:W${currentRow}`
      const fileName = `門市業績表_${dayjs().format('YYYYMMDD')}.xlsx`
      res.setHeader('Content-Type', 'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet')
