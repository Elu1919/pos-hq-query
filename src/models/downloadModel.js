...
const downloadModel = {
    posSaleToERP: async (filterIn) => {
        try {
          ...
          DECLARE @TYPE_STR        NVARCHAR(MAX) = @TYPES
          ...
          FROM (
              ...
              WHERE 
                  ...
                  AND (ISNULL(@SHOP_ID_STR, '') = '' OR S1.SHOP_ID IN (SELECT t.v.value('.', 'NVARCHAR(20)') FROM @ShopXml.nodes('/root/v') AS t(v)))
                  AND (ISNULL(@TYPE_STR, '') = '' OR S0.TYPE IN (SELECT t.v.value('.', 'NVARCHAR(20)') FROM @TypeXml.nodes('/root/v') AS t(v)))
          ) AS BaseData
