 ...
 * 負責渲染分頁控制項，並處理點擊事件。
 * @param {string} containerId - 放置分頁元件的 HTML 元素的 ID (例如 'vip-pagination-client')。
 * @param {number} totalItems - 總資料筆數。
 * @param {number} pageSize - 每頁顯示筆數。
 * @param {function} onPageChange - 頁碼改變時的回調函式，接收 (newPage) 作為參數。
 */
class Paginator {
  ...
  render() {
    ...
    html += `<li class="page-item ${this.currentPage === 1 ? 'disabled' : ''}">
                <a class="page-link" href="#" data-page="${this.currentPage - 1 > 0 ? this.currentPage - 1 : 1}" aria-label="Previous">
    ...
    html += `<li class="page-item ${this.currentPage === this.totalPages ? 'disabled' : ''}">
                <a class="page-link" href="#" data-page="${this.currentPage + 1 <= this.totalPages ? this.currentPage + 1 : this.totalPages}" aria-label="Next">
